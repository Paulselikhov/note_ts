...
function App() {
    ...
    const createNote = () => {
        const newNote = {
            ...NOTE_TEMPLATE,
            id: notes.length ? notes[0].id + 1 : 1,
        };
    ...
    const searchedNotes = useMemo(() => {
        return notes.filter((note) =>
            note.name.toLowerCase().includes(findState.toLocaleLowerCase())
        );
    }, [findState, notes]);
    ...
    return (
        <div className='App'>
            ...
            <div className=' NoteForm'>
                ...
                />
                {activeNote && (
                    <NoteEditForm note={activeNote} updateNote={updateNote} />
                )}
            </div>
        </div>
    );
